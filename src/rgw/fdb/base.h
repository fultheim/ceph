...
struct select final
{
 ...
 mutable struct {
  ...
  // I learn more, but for now this setting at least means it's "plumbed through":
  FDBStreamingMode streaming_mode = FDB_STREAMING_MODE_ITERATOR; // JFW: FDB_STREAMING_MODE_WANT_ALL; 
...
// JFW: I think that this actually just needs to request a new future and then loop, assigning back to the same handle--
// need to rework this (and ideally see if we can trigger it)-- and generalize all of these...
